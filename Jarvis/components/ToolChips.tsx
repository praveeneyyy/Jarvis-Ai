...
export default function ToolChips({ initialOpen = true }: ToolChipsProps) {
  ...
  return (
    <div className="min-h-[220px] w-full max-w-[20rem] pb-1">
      {/* collapsed run header */}
      <button
        type="button"
        aria-expanded={open}
        onClick={() => setOpen((current) => !current)}
        className="-mx-1.5 flex w-fit items-center gap-1.5 rounded-control px-1.5 py-1 text-[12.5px] text-ink-2 transition-colors duration-100 hover:bg-hover"
      >
          ...
          {ROWS.slice(0, step).map((row) => {
            ...
            <div key={row.label} style={{ animation: "fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" }}>
              <button
                ...
                className="group/row -mx-[3px] flex h-7 w-[calc(100%+6px)] min-w-0 items-center gap-2 rounded-control px-[3px] text-left transition-colors duration-100 hover:bg-hover"
              >
                ...
                <div className="min-h-0 overflow-hidden">
                  <div className="mt-0.5 mb-1 ml-2 flex flex-col gap-0.5 border-l border-line py-0.5 pl-3.5">
                    {row.detail.map((line) => (
                      <span
                        key={line.text}
                        className={`truncate text-[11.5px] leading-[1.6] ${row.detailMono ? "font-mono" : ""} ${line.tone === "add" ? "text-green-500" : "text-ink-2"}`}
                      >
                        {line.text}
                      </span>
                    ))}
        ...
        <div className="mt-2.5 flex max-w-full flex-wrap gap-1.5 border-t border-line pt-2.5">
          {DIFFS.map((d, i) => (
            ...
            >
              ...
              <span className="shrink-0 text-green-500 tabular-nums">+{d.add}</span>
              {d.del > 0 && <span className="shrink-0 text-red-500 tabular-nums">−{d.del}</span>}
            </span>
          ))}
          <button
            type="button"
