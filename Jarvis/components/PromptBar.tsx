...
export interface PromptBarProps {
  ...
  onSend?: (text: string, model: string, attachments: string[]) => void;
}

export default function PromptBar({ variant = "Rounded", onSend }: PromptBarProps) {
  ...
  const send = () => {
    ...
    if (onSend) {
      onSend(textToSend, model.name, attachments);
    }
    setDraft("");
    setAttachments([]);
    closeMenus();
                ...
                {source?.connect && (
                  <span
                    ...
                    className={`shrink-0 text-[12px] font-medium transition-colors duration-100 ${
                      connected ? "text-green-500" : "text-purple-400 hover:underline"
                    }`}
                  >
        ...
        >
          ...
          <button
            ...
            className={`flex size-7 shrink-0 items-center justify-center transition-[background-color,color,transform] duration-150 active:scale-[0.94] ${
              pill ? "rounded-full" : "rounded-[8px]"
            } ${listening ? "bg-purple-500/20 text-purple-400" : "text-ink-3 hover:bg-hover hover:text-ink"} ${
          ...
          <button
            ...
            style={{
              background: canSend ? "#da7756" : "var(--line-strong, #383832)",
              color: canSend ? "#ffffff" : "var(--ink-2, #75756d)",
            }}
          >
