    ...
    <div className="flex w-fit items-center gap-2.5">
      <span aria-hidden="true" className="grid grid-cols-[repeat(3,4px)] gap-[1.5px]">
        {delays.map((d, i) => (
          <span
            key={i}
            className={`size-1 bg-current ${round ? "rounded-full" : "rounded-[1px]"}`}
            style={{
      ...
      <span
        ...
        style={{
          backgroundImage:
            "linear-gradient(90deg, var(--ink-3, #888) 35%, var(--ink, #fff) 50%, var(--ink-3, #888) 65%)",
